...
class TestClientConfig:
    def test_uses_installed_app_section(self):
        assert set(client_config("key")) == {"installed"}

    def test_carries_bundled_client_id(self):
        assert client_config("key")["installed"]["client_id"] == CLIENT_ID

    def test_access_key_becomes_client_secret(self):
        assert client_config("the-key")["installed"]["client_secret"] == "the-key"

    def test_points_at_google_endpoints(self):
...
class TestAccessKeyHash:
    def test_is_sha256_digest(self):
