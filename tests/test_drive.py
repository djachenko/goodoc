...
@pytest.fixture
def drive(mock_creds: MagicMock) -> Drive:
    mock_auth = MagicMock()
...
class TestMimeMap:
    ...
    def test_extensions_map_to_google_formats(self, extension: str, expected_target: str) -> None:
...
class TestUpload:
    def test_unsupported_extension_exits(
            self,
            drive: Drive,
            tmp_path: Path,
    ...
    def test_creates_with_stem_name_and_target_mime(
            self,
            drive: Drive,
            docx_file: Path,
